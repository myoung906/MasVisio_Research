...
async function testMobileMenu() {
  ...
  try {
    ...
    const hamburgerButton = await page
      .locator(
        '.hamburger-menu, .menu-toggle, #menu-toggle, [class*="hamburger"], [class*="menu-button"]',
      )
      .first();
    ...
    if ((await hamburgerButton.count()) > 0) {
      ...
      const allButtons = await page
        .locator('button, [onclick], [class*="menu"], [class*="nav"]')
        .all();
    ...
    if ((await koLink.count()) > 0) {
      ...
      const koHamburgerButton = await page
        .locator(
          '.hamburger-menu, .menu-toggle, #menu-toggle, [class*="hamburger"], [class*="menu-button"]',
        )
        .first();
      ...
      if ((await koHamburgerButton.count()) > 0) {
        ...
        await page.screenshot({
          path: "ko_menu_opened_mobile.png",
          fullPage: true,
        });
      } else {

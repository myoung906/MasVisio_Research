...
## skillsmp search 스킬로 설치시 다음 지침 준수:
1. 특정 스킬 설치 시, 먼저 위험성 없는 격리 폴더에서 사전에 보안 검토하여 결과 보고할 것
2. 특정 스킬 설치 시, 1번의 보안 결과 보고와 더불어 사용자의 승인을 반드시 받을것
3. 신규 스킬즈 또는 스타 점수가 낮은 경우, 설치하지 말고 사용자에게 알려 보안에 주의할것
